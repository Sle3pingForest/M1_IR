...
import numpy as np
from repartition_tri import repartition_egal
...
def draw(data, name):
    ...
    pl = plt.figure(1)
    #print len(color_solve), "     ", len (color_solve[0])
    tab_repar = repartition_egal(color_solve, centreRef, 9, 'LAB')
    temp = color_solve
    tab_repar
    ...
    for i in range(0,6):
        ...
        for j in range(0, len(tab_repar[i])):
            yplacer = j+10
            p +=10
            color_res.append([round(tab_repar[i][j][0]/max_reading,3), round(tab_repar[i][j][1]/max_reading,3), round(tab_repar[i][j][2]/max_reading,3)])
            plt.scatter(i, p+yplacer , s=800, marker='o', c=color_res[k])
            k = k +1
...
draw(R3_bruitGris2,'R3_bruitGris2')
